...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for expense in expenses_to_check:
            try:
                ...
                if is_dry_run:
                    self.stdout.write(
                        self.style.WARNING(
                            f'[Dry Run] Found corrupted expense: ID={expense.id}, Title="{expense.title}"'
                        )
                    )
                else:
                    self.stdout.write(
                        self.style.WARNING(
                            f'Deleting expense {expense.id} ("{expense.title}") due to invalid paid_by link.'
                        )
                    )
                    expense.delete()
                    deleted_count += 1
        ...
        if is_dry_run:
            self.stdout.write(
                self.style.SUCCESS(
                    f"Dry run complete. Found {found_count} corrupted expenses that would be deleted."
                )
            )
        elif deleted_count > 0:
            self.stdout.write(
                self.style.SUCCESS(
                    f"Successfully deleted {deleted_count} corrupted expenses."
                )
            )
        else:
            self.stdout.write(
