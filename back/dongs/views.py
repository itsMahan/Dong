...
class AddDongMemberView(APIView):
    ...
    def post(self, request):
        ...
        if DongMember.objects.filter(
            ...
            return Response(
                "A member with this name already exists in this dong.",
...
class DeleteDongMember(APIView):
    ...
    def delete(self, request, dong_id, member_name):
        ...
        try:
            ...
            return Response(
                {"error": "No Dong Member has been found"},
                status=status.HTTP_404_NOT_FOUND,
            )
...
class AddExpenseView(APIView):
    ...
    def post(self, request, dong_id):
        ...
        serializer = ExpenseCreateSerializer(
            data=request.data, context={"dong_id": dong_id}
        )
        ...
        if serializer.is_valid():
            ...
            # ذخیره expense
            expense = serializer.save(dong=dong, created_by=request.user)
            ...
            return Response(
                {
                    ...
                    "budget_info": {
                        ...
                        "remaining_budget": (
                            round(dong.get_remaining_budget(), 2)
                            if dong.total_budget
                            else None
                        ),
...
class UpdateExpenseView(APIView):
    ...
    def patch(self, request, expense_id):
        ...
        if serializer.is_valid():
            ...
            include_tax = serializer.validated_data.get(
                "include_tax", expense.include_tax
            )
            ...
            if dong.total_budget is not None:
                ...
                remaining_budget = dong.get_remaining_budget() + Decimal(
                    str(old_expense_total)
                )
...
class DeleteExpenseView(APIView):
    ...
    def delete(self, request, pk):
        ...
        expense.delete()
        return Response(
            "expense has been deleted successfully", status=status.HTTP_200_OK
        )
...
class AddExpenseParticipantView(APIView):
    ...
    def post(self, request):
        ...
        if ExpenseParticipant.objects.filter(
            ...
            return Response(
                "An expense participant with this name already exists in this expense.",
                status=status.HTTP_400_BAD_REQUEST,
            )
        else:
            ExpenseParticipant.objects.create(
    ...
    def get(self, request, dong_id):
        ...
        for member in members:
            ...
            participated_expenses = ExpenseParticipant.objects.filter(
                member=member, expense__dong=dong
            )
        ...
        return Response({"dong_title": dong.title, "balances": balances})
...
class SettlementView(APIView):
    ...
    def get(self, request, dong_id):
        ...
        while creditor_idx < len(creditor_remaining) and debtor_idx < len(
            debtor_remaining
        ):
            ...
            # به‌روزرسانی مقادیر
            creditor_remaining[creditor_idx] = (
                creditor_name,
